...
export function createDocSTClothingSummary({ monada = '', data = {}, materialCatalog = [], onSave = null } = {}) {
  let state = normalize({ ...data, commonFields: { ...(data.commonFields || {}), monada: data.commonFields?.monada ?? monada } });
  return {
    renderEdit: () => renderDocSTEdit(state, materialCatalog),
...
export function renderDocSTPrint(data = {}) {
  ...
  return `<article class="print-document-area doc-st-print-page">
    ...
    <div class="doc-st-signatures"><div><span>ΘΕΩΡΗΘΗΚΕ</span><span>Ο</span><strong>Δκτής της Μονάδας</strong>${signature(state.specificFields.commander)}</div><div><span>ΣΤΓ ${escapeHtml(state.specificFields.stg)}</span><span>Ο</span><strong>ΔΧΣΤΗΣ ΥΛΙΚΟΥ</strong>${signature(state.specificFields.manager)}</div></div>
  </article>`;
}
