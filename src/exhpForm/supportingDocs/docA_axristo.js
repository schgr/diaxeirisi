...
function paginateMaterials(rows = []) { const source = (Array.isArray(rows) ? rows : []).filter(hasValue).map((row, index) => ({ ...row, seq: row.seq || index + 1 })); if (!source.length) source.push({ seq: 1 }); const pages = []; for (let index = 0; index < source.length; index += 17) pages.push(source.slice(index, index + 17)); return pages; }
