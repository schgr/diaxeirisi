...
const SHARE_FLAG_FIELDS = new Set([
  'requiresComposition',
  'requiresSerialNumber',
  'requiresWeaponRegistry',
  'requiresAmmunitionBatchBook',
  'requiresTrainingAmmunitionBatchBook',
  'requiresChangeSheet'
]);
...
function createSharesService(db) {
  ...
  return {
    ...
    updateShareDetails(id, payload) {
      ...
      if (payloadKeys.length && payloadKeys.every((key) => SHARE_FLAG_FIELDS.has(key))) {
        return mapShare(repository.updateShareFlags(shareId, {
          requiresComposition: payload.requiresComposition !== undefined
            ? Boolean(payload.requiresComposition)
            : Boolean(share.requires_composition),
          requiresSerialNumber: payload.requiresSerialNumber !== undefined
            ? Boolean(payload.requiresSerialNumber)
            : Boolean(share.requires_serial_number),
          requiresWeaponRegistry: payload.requiresWeaponRegistry !== undefined
            ? Boolean(payload.requiresWeaponRegistry)
            : Boolean(share.requires_weapon_registry),
          requiresAmmunitionBatchBook: payload.requiresAmmunitionBatchBook !== undefined
            ? Boolean(payload.requiresAmmunitionBatchBook)
            : Boolean(share.requires_ammunition_batch_book),
          requiresTrainingAmmunitionBatchBook: payload.requiresTrainingAmmunitionBatchBook !== undefined
            ? Boolean(payload.requiresTrainingAmmunitionBatchBook)
            : Boolean(share.requires_training_ammunition_batch_book),
          requiresChangeSheet: payload.requiresChangeSheet !== undefined
    ...
    saveChangeSheet(id, entries) {
      ...
      const cleanEntries = (Array.isArray(entries) ? entries : []).map((entry) => {
        ...
        const compositionImportOpening = notes === 'COMPOSITION_IMPORT_OPENING';
        if (
          !changeDate ||
          ...
          quantity < 0 ||
          (quantity === 0 && !compositionImportOpening)
        ) {
          throw new AppError(
