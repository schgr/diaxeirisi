...
export async function renderSettingsPage(container, settingsApi, clothingApi, showToast, initialTab = '', sharesApi = window.appApi.shares) {
  const [settings, authStatus, backups, appVersion, addyReferenceData] = await Promise.all([
    settingsApi.get(),
  ...
  container.innerHTML = `
    ...
    <div class="transaction-tab-panel" data-settings-panel="general" hidden>
      <div class="settings-layout">
        <section class="page-panel">
          ...
          <form id="service-form" class="stacked-form autosave-form" data-no-page-draft>
        ...
        <section class="page-panel">
          ...
          <form id="officers-form" class="stacked-form autosave-form" data-no-page-draft>
    ...
    <div class="transaction-tab-panel" data-settings-panel="parameters" hidden>
      <nav class="transaction-flow-home contextual-tile-menu settings-subtab-menu" data-settings-submenu aria-label="Ενότητες δεδομένων συστήματος">
        <button class="home-tile transaction-flow-tile" data-settings-subtab="initial-records" type="button"><span class="home-tile-icon">ΑΚ</span><span class="home-tile-title">Αρχικές Καταχωρήσεις</span><span class="home-tile-code">§ ΔΣ-1</span></button>
        <button class="home-tile transaction-flow-tile" data-settings-subtab="system-catalogs" type="button"><span class="home-tile-icon">ΚΣ</span><span class="home-tile-title">Κατάλογοι Συστήματος</span><span class="home-tile-code">§ ΔΣ-2</span></button>
        <button class="home-tile transaction-flow-tile" data-settings-subtab="material-card-settings" type="button"><span class="home-tile-icon">ΡΚ</span><span class="home-tile-title">Ρυθμίσεις Καρτέλας Υλικού</span><span class="home-tile-code">§ ΔΣ-3</span></button>
      </nav>
      ...
      <div class="settings-layout parameters-settings-layout" data-settings-subpanel="material-card-settings" hidden>
        <section class="page-panel wide-panel">
          ...
          <div data-material-card-flags-mount><p class="muted">Οι μερίδες θα φορτωθούν όταν ανοίξει αυτή η ενότητα.</p></div>
        </section>
      </div>
    </div>
      ...
      <div class="settings-layout security-settings-layout">
        <section class="page-panel">
          ...
          <form id="change-password-form" class="stacked-form">
            ...
            <label class="field"><span>Επιβεβαίωση νέου κωδικού</span><input name="confirmation" type="password" minlength="10" autocomplete="new-password" required /></label>
  ...
  bindSettingsSubtabs(container, sharesApi, showToast);
      ...
      <table class="index-table material-card-flags-table">
        ...
        <tbody>${shares.length ? shares.map((share, index) => `
          <tr data-material-card-flags="${share.id}">
            ...
            <td><input data-material-flag="requiresComposition" data-no-page-draft type="checkbox" ${share.requiresComposition ? 'checked' : ''} aria-label="Σύνθεση Υλικού ${escapeHtml(share.shareNumber)}" /></td>
            <td><input data-material-flag="requiresSerialNumber" data-no-page-draft type="checkbox" ${share.requiresSerialNumber ? 'checked' : ''} aria-label="Σειριακός Αριθμός ${escapeHtml(share.shareNumber)}" /></td>
            <td><input data-material-flag="requiresWeaponRegistry" data-no-page-draft type="checkbox" ${share.requiresWeaponRegistry ? 'checked' : ''} aria-label="Μητρώο Οπλισμού ${escapeHtml(share.shareNumber)}" /></td>
            <td><input data-material-flag="requiresAmmunitionBatchBook" data-no-page-draft type="checkbox" ${share.requiresAmmunitionBatchBook ? 'checked' : ''} aria-label="Πυρομαχικά Β.Φ. ${escapeHtml(share.shareNumber)}" /></td>
            <td><input data-material-flag="requiresTrainingAmmunitionBatchBook" data-no-page-draft type="checkbox" ${share.requiresTrainingAmmunitionBatchBook ? 'checked' : ''} aria-label="Πυρομαχικά Εκπαιδεύσεως ${escapeHtml(share.shareNumber)}" /></td>
          </tr>
...
function bindSettingsSubtabs(container, sharesApi, showToast) {
  ...
  panel.querySelectorAll('[data-settings-subtab]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      if (subtab === 'material-card-settings') {
        await loadMaterialCardSettings(panel, sharesApi, showToast);
      }
    });
  });
}

async function loadMaterialCardSettings(panel, sharesApi, showToast) {
  const mount = panel.querySelector('[data-material-card-flags-mount]');
  if (!mount || mount.dataset.loaded === 'true' || mount.dataset.loading === 'true') return;
  mount.dataset.loading = 'true';
  mount.innerHTML = '<p class="muted">Φόρτωση μερίδων…</p>';
  try {
    const shares = await sharesApi.list();
    mount.innerHTML = renderMaterialCardFlags(shares);
    mount.dataset.loaded = 'true';
    bindMaterialCardFlagEvents(mount, sharesApi, showToast);
  } catch (error) {
    mount.innerHTML = '<p class="muted">Δεν ήταν δυνατή η φόρτωση των μερίδων.</p>';
...
function bindSettingsEvents(container, settingsApi, clothingApi, sharesApi, showToast) {
  ...
  container.addEventListener('click', async (event) => {
    ...
    if (deleteButton) {
      ...
      if (!(await confirmDialog({ message: 'Να διαγραφεί η επιχείρηση τιμολογίου;' }))) return;
      try {
  ...
  container.querySelector('[data-backup-restore]')?.addEventListener('click', async () => {
    if (!(await confirmDialog({ message: 'Η εφαρμογή θα επανεκκινηθεί και τα τρέχοντα δεδομένα θα αντικατασταθούν. Θα δημιουργηθεί πρώτα αντίγραφο ασφαλείας. Συνέχεια;' }))) return;
    try {
...
function bindMaterialCardFlagEvents(container, sharesApi, showToast) {
...
function bindInitialInventoryEvents(container, settingsApi, showToast) {
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    if (!(await confirmDialog({ message: 'Η εισαγωγή θα δημιουργήσει νέες μερίδες και θα ενημερώσει τις υπάρχουσες βάσει του αριθμού μερίδας. Να συνεχιστεί;' }))) return;
...
function bindCompositionImportEvents(container, settingsApi, showToast) {
  ...
  importButton?.addEventListener('click', async () => {
    ...
    if (!(await confirmDialog({ message: 'Οι συνθέσεις των μερίδων που περιλαμβάνονται στο Excel θα αντικατασταθούν. Να συνεχιστεί;' }))) return;
...
function bindClothingSettings(container, clothingApi, showToast, initialItems) {
  ...
  section.addEventListener('click', async (event) => {
    ...
    if (event.target.closest('[data-delete-clothing]')) {
      if (!(await confirmDialog({ message: 'Να διαγραφεί το είδος ιματισμού;' }))) return;
      try {
