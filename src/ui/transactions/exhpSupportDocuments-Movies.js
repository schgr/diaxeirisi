...
export function renderExhpSupportChecklist(container, templates, draftSupports = new Map()) {
...
function renderManualExhpSupportRow(value = '') {
  return `
    <label class="exhp-support-row" data-manual-exhp-support-row>
      <span><strong>Δικαιολογητικό</strong>Τίτλος / αριθμός / ημερομηνία / στοιχεία</span>
...
export function renderSupportTemplateCards(referenceData, reasonName) {
  ...
  const templates = referenceData.exhpSupportTemplates.filter((item) => isSameIssueReason(item.issueReason, reasonName));
  if (!templates.length) {
  ...
  modal.innerHTML = `
    <section class="request-document-modal support-form-modal">
      ...
      <div class="support-template-form-state no-print" data-support-template-form>
        ...
        ${supportHasMaterialTable(faithfulDefinition, code, support) ? renderSupportEditableMaterials(initialItems) : ''}
...
function renderSupportEditableMaterials(items) {
  return `
    <section class="official-editable-materials">
      <h3>Υλικά</h3>
      <div class="table-wrap"><table>
        <thead><tr><th>Α/Α</th><th>Αρ. Μερίδας</th><th>Αρ. Ονομαστικού</th><th>Περιγραφή</th><th>ΜΜ</th><th>Ποσότητα</th><th>Παρ/σεις</th><th></th></tr></thead>
        <tbody data-support-material-items>${items.map((item, index) => renderSupportMaterialRow(item, index)).join('')}</tbody>
      </table></div>
...
function renderSupportMaterialRow(item = {}, index = 0) {
  return `<tr>
    <td data-support-row-number>${index + 1}</td>
    <td><input data-support-item-field="shareNumber" value="${escapeHtml(item.shareNumber || '')}" /></td>
    <td><input data-support-item-field="nominalNumber" value="${escapeHtml(item.nominalNumber || item.nomenclatureNumber || '')}" ${item.shareNumber ? 'readonly' : ''} /></td>
    <td><input data-support-item-field="description" value="${escapeHtml(item.description || '')}" ${item.shareNumber ? 'readonly' : ''} /></td>
    <td><input data-support-item-field="measurementUnit" value="${escapeHtml(item.measurementUnit || item.unit || '')}" ${item.shareNumber ? 'readonly' : ''} /></td>
...
function bindSupportEditableMaterials(formPanel, showToast) {
  formPanel.addEventListener('click', (event) => {
    if (event.target.closest('[data-add-support-material]')) {
      ...
      body.insertAdjacentHTML('beforeend', renderSupportMaterialRow({}, body.querySelectorAll('tr').length));
      return;
    }
...
function renderSupportApplicationFields(definition, formData, documentData, support) {
  ...
  return `
    <section class="support-application-fields no-print">
      ...
      <div class="support-form-fields">
        ${supportFormField('Αριθμός και ημερομηνία', 'documentReference', formData.documentReference || support.documentReference)}
        ${hasSplitDate ? supportFormField('Ημερομηνία', 'documentDate', getCombinedSupportDate(formData), false, 'date') : ''}
        ${visibleFields.map((field) => supportFormField(
          ...
          isSupportDateField(field.name) ? 'date' : 'text'
        )).join('')}
      </div>
    </section>
  `;
