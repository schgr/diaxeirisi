...
export async function maybeSuggestShareNumber(transactionsApi, referenceData, controls, showToast) {
  ...
  const accepted = await confirmDialog({
    message: `Δεν βρέθηκε αντιστοιχία μερίδας. Προτείνεται ο αριθμός ${suggestion.shareNumber}. Θέλεις να χρησιμοποιηθεί;`
  });
...
export async function confirmFutureTransactionDate(documentDate) {
  ...
  return await confirmDialog({
    message: `Η κίνηση αυτή θα καταχωρηθεί στο επόμενο οικονομικό έτος (${selectedYear}). Θέλετε να συνεχίσετε;`
  });
}
