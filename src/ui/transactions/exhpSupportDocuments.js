...
export function renderSupportTemplateCards(referenceData, reasonName) {
  ...
  return templates.map((template) => `
    <article class="exhp-support-row">
      <span><strong>${escapeHtml(template.documentCode || 'Έντυπο')}</strong>${escapeHtml(template.title)}</span>
      <button class="secondary-button" data-open-support-template="${template.id}" type="button">${isInventorySupportTemplate(template) ? 'Άνοιγμα Απογραφών' : 'Συμπλήρωση Εντύπου'}</button>
    </article>
  `).join('');
}
        ...
        ${documentData.supports.length ? documentData.supports.map((support) => `
          <label class="exhp-support-row" data-saved-support="${support.id}">
            ...
            <div class="row-actions">
              ${support.printable ? `<button class="secondary-button" data-print-support="${support.id}" type="button">${isInventorySupportTemplate(support) ? 'Άνοιγμα Απογραφών' : 'Έντυπο'}</button>` : ''}
