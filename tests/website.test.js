...
const { displayVersion, version } = require('../package.json');
const releaseVersion = displayVersion || version;
...
assert.match(html, new RegExp(`Έκδοση ${releaseVersion.replace(/\./g, '\\.')}`));
...
assert.match(script, new RegExp(`diaxeirisi-Ylikoy-Windows-10-11-x64-Setup-${releaseVersion.replace(/\./g, '\\.')}\\.exe`));
